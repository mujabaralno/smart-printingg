...
const StepIndicator: FC<StepIndicatorProps> = ({ activeStep, quoteMode }) => {
  ...
  const getColors = () => {
    if (quoteMode === "existing") {
      return {
        active:
          "bg-gradient-to-br from-[#ea078b] to-[#d4067a] text-white shadow-lg shadow-pink-200",
        glow: "bg-pink-400",
        text: "text-pink-700",
        arrow: "text-pink-500",
      };
        ...
        <div className="flex items-center justify-center w-full">
          {(() => {
            ...
            const startIndex = Math.max(
              0,
              Math.min(activeStep - 2, steps.length - 3)
            );
            ...
            return visibleSteps.map((step, index) => {
              ...
              return (
                ...
                >
                  {/* Step Circle */}
                  <div
                    className={`group relative flex items-center justify-center ${circleSize} rounded-full font-bold ${textSize} transition-all duration-300 ${
                      isCompleted
                  ...
                  >
                    ...
                    {isActive && (
                      <div
                        className={`absolute inset-0 rounded-full ${colors.glow} animate-ping opacity-20`}
                      />
                    )}
                  </div>
                  ...
                  {/* Step Label */}
                  <p
                    className={`${labelSize} ${labelWeight} text-center mt-2 transition-all duration-300 ${
                      isActive
                        ? colors.text
                        : isCompleted
                        ? "text-green-700"
          ...
          return (
            <div key={step.number} className="flex flex-row items-center">
              ...
              <div className="flex flex-col items-center">
                {/* Step Circle */}
                <div
                  className={`group relative flex items-center justify-center w-16 h-16 rounded-full font-bold text-xl transition-all duration-300 hover:scale-110 ${
                    isCompleted
                ...
                {/* Step Label */}
                <p
                  className={`text-base text-center font-semibold mt-3 transition-all duration-300 ${
                    isActive
              ...
              {index < steps.length - 1 && (
                <div
                  className={`w-20 h-0.5 mx-6 transition-all duration-300 ${
