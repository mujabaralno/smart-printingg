...
import { CheckCircle, Loader2, XCircle, MoreHorizontal, Pencil, Eye } from "lucide-react";
...
function StatusPill({ status }: { status: QuoteRow["status"] }) {
  ...
  return <Badge className="bg-slate-100 text-slate-700 border rounded-full px-2.5 py-1 text-[11px] font-medium">{status}</Badge>;
}
        ...
        <Table>
          <TableHeader className="bg-slate-50">
            <TableRow className="border-slate-200">
              <TableHead className="w-40 text-slate-600 text-md font-semibold">Quote ID</TableHead>
              <TableHead className="w-64 text-slate-600 text-md font-semibold">Client Name</TableHead>
              <TableHead className="w-64 text-slate-600 text-md font-semibold">Product</TableHead>
              <TableHead className="w-32 text-slate-600 text-md font-semibold">Date</TableHead>
              <TableHead className="w-32 text-slate-600 text-md font-semibold">Amount</TableHead>
              <TableHead className="w-32 text-slate-600 text-md font-semibold">Status</TableHead>
              <TableHead className="w-10 text-right text-slate-600 text-md font-semibold"> </TableHead>
            </TableRow>
          </TableHeader>
            ...
            <TableBody>
              {pageData.map((row) => (
                <TableRow key={row.id} className="hover:bg-slate-50">
                  ...
                  <TableCell className="text-slate-900 font-semibold">AED {fmtMoney(row.totalAmount)}</TableCell>
                  ...
                  <TableCell className="text-right">
                    <DropdownMenu modal={false}>
                      ...
                      <DropdownMenuContent align="end" className="w-40 bg-white" onCloseAutoFocus={(e)=>e.preventDefault()}>
                        <DropdownMenuLabel>Actions</DropdownMenuLabel>
                        <DropdownMenuItem onSelect={(e)=>{e.preventDefault(); onEdit(row);}}>
                          <Pencil className="h-4 w-4 mr-2" /> Edit
                        </DropdownMenuItem>
                        <DropdownMenuItem onSelect={(e)=>{e.preventDefault(); onView(row);}}>
