...
const UAE_AREAS = [
  ...
  {
    name: "Umm Al Quwain Industrial Area",
    state: "Umm Al Quwain",
    country: "UAE",
  },
  ...
  {
    name: "Ras Al Khaimah Industrial Area",
...
export default function ClientManagementPage() {
  ...
  const filtered = React.useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      const hitClientType =
        clientTypeFilter === "all" || r.clientType === clientTypeFilter;
  ...
  const tableData: ClientRow[] = (filtered ?? []).map((r: any) => ({
    id: r.id,
    clientType: r.clientType === "Individual" ? "Individual" : "Company",
    companyName: r.companyName,
  ...
  const onSubmit = async () => {
    ...
    if (draft.clientType === "Company") {
      if (
        ...
        return alert(
          "Please fill Company Name, Contact Person, Email, and Role for Company clients."
        );
      }
    } else {
      if (!draft.firstName || !draft.lastName || !draft.email) {
        return alert(
          "Please fill First Name, Last Name, and Email for Individual clients."
        );
      }
    }
    ...
    try {
      if (mode === "add") {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.details || errorData.error || "Failed to update client"
          );
        }
  ...
  const updateContactPerson = (field: keyof ClientRow, value: string) => {
    ...
    if (field === "firstName" || field === "lastName") {
      newDraft.contactPerson = `${newDraft.firstName || ""} ${
        newDraft.lastName || ""
      }`.trim();
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex gap-5">
          ...
          <div>
            ...
            <p className="text-gray-600 mt-2">
              Manage your client database. Add new clients, update information,
              and track client relationships.
            </p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <div className="text-center">
              ...
              {loading ? (
                ...
                <div className="text-lg font-bold text-slate-900">
                  {filtered.filter((c) => c.clientType === "Company").length}
                </div>
              )}
            </div>
            <div className="text-center">
              ...
              {loading ? (
                ...
                <div className="text-lg font-bold text-slate-900">
                  {filtered.filter((c) => c.clientType === "Individual").length}
                </div>
              )}
            </div>
          </div>
        ...
        <div className="flex md:hidden w-full">
          ...
          <Button
            ...
            className="bg-[#27aae1] md:flex hidden hover:bg-[#1e8bc3] text-white px-6 py-3 rounded-xl font-medium shadow-lg hover:shadow-xl transition-all duration-200 h-12 w-full sm:w-auto"
          >
        ...
        <div className="w-full p-1 md:p-4 space-y-6 bg-white shadow-sm border border-slate-200 rounded-2xl">
          <div className="hidden md:grid-cols-2 w-full gap-5 md:grid">
            {/* Filters */}
          ...
          <div className="p-0">
            ...
            <div className="hidden lg:block overflow-x-auto">
              <ClientTable
                ...
                onEdit={(row) =>
                  onEdit(filtered.find((r: any) => r.id === row.id) ?? row)
                }
              />
            </div>
                  ...
                  >
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center justify-between">
                        <span className="font-mono text-sm font-medium text-slate-900 bg-blue-100 px-3 py-2 rounded-lg">
                ...
                <>
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateContactPerson("companyName", e.target.value)
                      }
                ...
                <>
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateContactPerson("firstName", e.target.value)
                      }
                      className="border-slate-300 focus:border-[#ea078b] focus:ring-[#ea078b] rounded-xl"
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateContactPerson("lastName", e.target.value)
                      }
                      className="border-slate-300 focus:border-[#ea078b] focus:ring-[#ea078b] rounded-xl"
                    />
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  {hasNoTrn !== 1 && (
                    <Input
                      ...
                      value={draft.trn || ""}
                      onChange={(e) =>
                        setDraft({ ...draft, trn: e.target.value })
                      }
                      className="border-slate-300 focus:border-[#ea078b] focus:ring-[#ea078b] rounded-xl"
                    />
                  )}
