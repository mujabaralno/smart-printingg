...
export class DatabaseService {
  ...
  static async createQuoteWithDetails(quoteData: {
    ...
    clientId: string;
    userId?: string;
    salesPersonId?: string; // Add salesPersonId field
    product: string;
    quantity: number;
    sides: string;
    printing: string;
    ...
    const { papers, finishing, amounts, operational, colors, userId, salesPersonId, approval, email, ...basicQuoteData } = quoteData;
    ...
    // Add salesPersonId if it's provided and not empty
    if (salesPersonId && salesPersonId.trim() !== '') {
      quoteDataToCreate.salesPersonId = salesPersonId;
    }
  ...
  static async getAllSalesPersons() {
    try {
      // Use direct SQLite query since Prisma has type conflicts with date fields
      const { execSync } = require('child_process');
      const path = require('path');
      const dbPath = path.join(process.cwd(), 'prisma', 'dev.db');
      
      const result = execSync(`sqlite3 "${dbPath}" "SELECT * FROM SalesPerson ORDER BY createdAt DESC;"`, { encoding: 'utf8' });
      ...
      console.log(`✅ DatabaseService: Found ${salesPersons.length} sales persons`);
      return salesPersons;
    } catch (error) {
      console.error('❌ Error fetching sales persons from DatabaseService:', error);
      return [];
    }
  }
