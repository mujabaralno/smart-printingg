...
async function migrateLocalToVercel() {
  console.log('🚀 Starting corrected migration from local SQLite to Vercel PostgreSQL...');
  ...
  try {
    ...
    // 5. MIGRATE QUOTES (with production schema compatibility)
    console.log('\n📋 Migrating quotes...');
    ...
    for (const quote of quotes) {
      try {
        // Create quote data compatible with production schema
        const quoteData = {
        ...
        // Add optional fields if they exist in production schema
        if (quote.productName !== undefined) quoteData.productName = quote.productName;
        if (quote.printingSelection !== undefined) quoteData.printingSelection = quote.printingSelection;
        if (quote.flatSizeHeight !== undefined) quoteData.flatSizeHeight = quote.flatSizeHeight;
        if (quote.closeSizeSpine !== undefined) quoteData.closeSizeSpine = quote.closeSizeSpine;
        if (quote.useSameAsFlat !== undefined) quoteData.useSameAsFlat = quote.useSameAsFlat;
        if (quote.flatSizeWidth !== undefined) quoteData.flatSizeWidth = quote.flatSizeWidth;
        if (quote.closeSizeHeight !== undefined) quoteData.closeSizeHeight = quote.closeSizeHeight;
        if (quote.closeSizeWidth !== undefined) quoteData.closeSizeWidth = quote.closeSizeWidth;
    ...
    for (const paper of papers) {
      try {
        ...
        if (paper.inputWidth !== undefined) paperData.inputWidth = paper.inputWidth;
        if (paper.inputHeight !== undefined) paperData.inputHeight = paper.inputHeight;
        if (paper.pricePerPacket !== undefined) paperData.pricePerPacket = paper.pricePerPacket;
        if (paper.pricePerSheet !== undefined) paperData.pricePerSheet = paper.pricePerSheet;
        if (paper.sheetsPerPacket !== undefined) paperData.sheetsPerPacket = paper.sheetsPerPacket;
        if (paper.recommendedSheets !== undefined) paperData.recommendedSheets = paper.recommendedSheets;
        if (paper.enteredSheets !== undefined) paperData.enteredSheets = paper.enteredSheets;
        if (paper.outputWidth !== undefined) paperData.outputWidth = paper.outputWidth;
        if (paper.outputHeight !== undefined) paperData.outputHeight = paper.outputHeight;
        if (paper.selectedColors !== undefined) paperData.selectedColors = paper.selectedColors;
    ...
    for (const finishing of finishings) {
      try {
        ...
        if (finishing.cost !== undefined) finishingData.cost = finishing.cost;

        await vercelPrisma.finishing.upsert({
          where: { id: finishing.id },
          update: finishingData,
          create: {
            id: finishing.id,
            ...finishingData
          }
