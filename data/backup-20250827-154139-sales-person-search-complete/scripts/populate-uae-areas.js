...
async function populateUAEAreas() {
  try {
    console.log('🌍 Populating UAE Areas data...');
    
    // Clear existing data
    await prisma.uAEArea.deleteMany({});
    console.log('✅ Cleared existing UAE areas data');
    
    // Insert new data
    const createdAreas = await prisma.uAEArea.createMany({
      data: uaeAreas
    });
    
    console.log(`✅ Created ${createdAreas.count} UAE areas`);
    
    // Show some sample data
    const sampleAreas = await prisma.uAEArea.findMany({
      take: 10,
      orderBy: { name: 'asc' }
    });
