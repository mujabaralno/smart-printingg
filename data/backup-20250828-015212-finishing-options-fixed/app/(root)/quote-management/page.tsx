...
export default function QuoteManagementPage() {
  ...
  const [clientId, setClientId] = React.useState<string>(""); // New client ID filter
  ...
  React.useEffect(() => {
    const loadQuotes = async () => {
      try {
        ...
        // Direct database query to get quotes with client data
        ...
        if (response.ok) {
          ...
          console.log('Raw quotes from direct database query:', quotesData);
          
          // Transform database quotes to match Row format
          const transformedQuotes = quotesData.map((quote: any) => {
            // Calculate amount from the amounts array
            let totalAmount = 0;
            ...
            // Get proper client name from direct database data
            let clientName = "Unknown Client";
            let contactPerson = "Unknown Contact";
            
            if (quote.client) {
              if (quote.client.companyName && quote.client.companyName.trim() !== "") {
                ...
                clientName = quote.client.contactPerson;
              } else if (quote.client.email && quote.client.email.trim() !== "") {
                clientName = quote.client.email;
              } else {
                clientName = `Client ${quote.quoteId}`;
              }
              
              // Set contact person
              if (quote.client.contactPerson && quote.client.contactPerson.trim() !== "") {
                contactPerson = quote.client.contactPerson;
              } else if (quote.client.firstName && quote.client.lastName) {
                contactPerson = `${quote.client.firstName} ${quote.client.lastName}`;
              } else if (quote.client.email && quote.client.email.trim() !== "") {
                contactPerson = quote.client.email;
              } else {
              ...
              clientName = `Client ${quote.quoteId}`;
              contactPerson = `Client ${quote.quoteId}`;
            }
  ...
  const filtered = React.useMemo(() => {
    const filteredQuotes = rows.filter((q) => {
      ...
      const hitClientId = clientId === "" || q.originalClientId === clientId;

      const hitFrom = from === "" || q.date >= from;
      const hitTo = to === "" || q.date <= to;

      return hitSearch && hitStatus && hitContactPerson && hitAmount && hitClientId && hitFrom && hitTo;
    });
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-sm font-medium text-blue-800">
                  Filtered by Client: {clientName || clientId}
                </span>
              </div>
              <Button
                variant="ghost"
