...
const Step4Operational: FC<Step4Props> = ({ formData, setFormData }) => {
  ...
  // ===== Helper function to convert color input to CSS color =====
  const getColorFromInput = (colorInput: string): string => {
    ...
    // Handle Pantone-like codes (just show as transparent for now)
    if (/^[0-9]{3,4}[A-Z]$/i.test(input)) {
      return 'transparent'; // Pantone codes - no direct color mapping
    }
      ...
      {formData.products.map((product, productIndex) => (
        <div key={productIndex} className="space-y-8">
          ...
          {product.papers.map((paper, paperIndex) => {
            ...
            {/* Optional Color Codes Section - Simple and Non-Prominent */}
            <div className="bg-slate-50 rounded-lg border border-slate-200 p-3 shadow-sm">
              <h6 className="text-sm font-medium text-slate-600 flex items-center mb-2">
                <Palette className="w-3 h-3 mr-2 text-slate-500" />
                Color Codes (Optional)
                <span className="ml-2 text-xs text-slate-400 font-normal">Pantone, Hex, or color names</span>
              </h6>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Input
                    type="text"
                    placeholder="Enter color code (e.g., #FF0000, Pantone 185C, Red)"
                  ...
                  <Button
                    ...
                    disabled={!colorInputs[productIndex]?.[paperIndex]?.trim()}
                  >
                    Add
                  </Button>
                </div>
                
                {/* Display Added Colors with Visualization */}
                {paperColors[productIndex]?.[paperIndex]?.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    {paperColors[productIndex][paperIndex].map((colorCode, colorIndex) => (
                      <div key={colorIndex} className="flex items-center gap-2 px-3 py-2 bg-white rounded-lg border border-slate-200 shadow-sm">
                        {/* Color Visualization */}
                        <div 
                          className="w-4 h-4 rounded border border-slate-300 shadow-sm"
                          style={{ 
                          ...
                          title={`Visualization of: ${colorCode}`}
                        ></div>
                ...
                {/* Help Text */}
                <div className="text-xs text-slate-500">
                  💡 Enter hex codes (#FF0000), Pantone codes (185C), or color names (Red, Blue, etc.)
                </div>
              </div>
            </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="bg-white rounded-xl p-4 border border-green-200 shadow-sm">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between">
                        <span className="text-slate-600">Items per Piece:</span>
                        <span className="font-semibold text-blue-600">
                          {opPaper?.inputWidth && opPaper?.inputHeight && outputDimensions[productIndex]?.width && outputDimensions[productIndex]?.height ? 
                            Math.floor(52 / outputDimensions[productIndex].width) * Math.floor(72 / outputDimensions[productIndex].height) : "–"}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-600">Total Output Items:</span>
                        <span className="font-semibold text-green-600">
                          {opPaper?.inputWidth && opPaper?.inputHeight && outputDimensions[productIndex]?.width && outputDimensions[productIndex]?.height ? 
                            (calculateCutPieces(opPaper.inputWidth, opPaper.inputHeight, 52, 72).totalPieces * 
                             Math.floor(52 / outputDimensions[productIndex].width) * Math.floor(72 / outputDimensions[productIndex].height)) : "–"}
