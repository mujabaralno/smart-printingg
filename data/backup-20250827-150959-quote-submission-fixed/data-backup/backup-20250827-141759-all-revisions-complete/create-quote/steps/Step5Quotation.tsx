...
const Step5Quotation: FC<Step5Props> = ({
  ...
  const [selectedSalesPersonId, setSelectedSalesPersonId] = React.useState<string>('');
    ...
    const loadSalesPersons = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const salesPersonsData = users
            .filter((user: any) => user.isSalesPerson)
            .map((user: any) => ({
              id: user.id,
          ...
          // Auto-select first sales person if none selected
          if (salesPersonsData.length > 0 && !selectedSalesPersonId) {
            setSelectedSalesPersonId(salesPersonsData[0].salesPersonId);
          }
        }
      } catch (error) {
                  ...
                  >
                    ...
                    <SelectContent>
                      {salesPersons.map((person) => (
                        <SelectItem key={person.salesPersonId} value={person.salesPersonId}>
                          <div className="flex items-center space-x-2">
                            <User className="h-4 w-4 text-slate-500" />
                            <span>{person.name}</span>
                            <span className="text-xs text-slate-500">({person.salesPersonId})</span>
                          </div>
                        </SelectItem>
                      ))}
                    </SelectContent>
