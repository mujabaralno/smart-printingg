...
const Step3ProductSpec: FC<Step3Props> = ({ formData, setFormData }) => {
  ...
  const calculateCosting = (product: Product) => {
    if (!digitalPricing || !offsetPricing) return;
    
    try {
      // Calculate digital costing
      const digitalResults = calcDigitalCosting({
        piece: { w: product.flatSize.width || 0, h: product.flatSize.height || 0 },
        quantity: product.quantity,
        sides: parseInt(product.sides),
        colors: { front: parseInt(product.colors.front) || 0, back: parseInt(product.colors.back) || 0 },
      ...
      const offsetResult = calcOffsetCosting({
        ...
        press: { w: 35, h: 50 }, // 35×50 press
        piece: { w: product.flatSize.width || 0, h: product.flatSize.height || 0 },
        quantity: product.quantity,
        sides: parseInt(product.sides) as 1 | 2,
        colorsF: parseInt(product.colors.front) || 0 as 1 | 2 | 4,
        colorsB: parseInt(product.colors.back) || 0 as 1 | 2 | 4,
                ...
                <div className="flex space-x-4">
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      name="printingMethod"
                      value="digital"
                      checked={formData.products[0]?.printingSelection === "Digital"}
                      onChange={() => updateProduct(0, { printingSelection: "Digital" })}
                      className="w-4 h-4 text-[#27aae1] border-gray-300 focus:ring-[#27aae1]"
                    />
                    <Label htmlFor="digital" className="text-sm font-medium text-slate-700">
                      Digital
                    </Label>
                  </div>
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      name="printingMethod"
                      value="offset"
                      checked={formData.products[0]?.printingSelection === "Offset"}
                      onChange={() => updateProduct(0, { printingSelection: "Offset" })}
                      className="w-4 h-4 text-[#27aae1] border-gray-300 focus:ring-[#27aae1]"
                    />
                    <Label htmlFor="offset" className="text-sm font-medium text-slate-700">
                      Offset
                    </Label>
                  </div>
                </div>
                      ...
                      >
                        <div className="flex justify-between items-center">
                          ...
                          <div className="text-right">
                            <div className="font-semibold text-slate-800">${result.total.toFixed(2)}</div>
                            <div className="text-xs text-slate-600">
                              Paper: ${result.paper.toFixed(2)} • Clicks: ${result.clicks.toFixed(2)}
                            </div>
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      <div className="flex justify-between items-center">
                        ...
                        <div className="text-right">
                          <div className="font-semibold text-slate-800">${offsetCostingResult.total.toFixed(2)}</div>
                          <div className="text-xs text-slate-600">
                            Paper: ${offsetCostingResult.paper.toFixed(2)} • Plates: ${offsetCostingResult.plates.toFixed(2)}
                          </div>
                        </div>
                      </div>
                    </div>
